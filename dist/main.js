...
class ASMOSApp {
    ...
    bindEvents() {
        ...
        document.getElementById('mm-textarea')?.addEventListener('input', (e) => { this.state.mindMapMarkdown = e.target.value; ASMOSStorage.saveState(this.state); this.updateMindMap('goal'); });
        ...
        document.addEventListener('click', (e) => { if (!e.target.closest('.markmap-node') && !e.target.closest('.node-actions-menu'))
            this.hideNodeMenu(); });
    }
    initDragAndDrop() {
    ...
    renderDashboard() {
        ...
        if (hpDashValue)
            hpDashValue.innerText = this.state.userHP.toString();
        const hpBarFill = document.getElementById('hp-bar-fill');
        if (hpBarFill)
            hpBarFill.style.width = `${Math.min(100, Math.max(0, this.state.userHP))}%`;
    ...
    renderTimezone() {
        ...
        Object.keys(TIME_SLOTS).forEach(slotId => {
            ...
            if (pCol && aCol && el) {
                ...
                const cap = (cfg.endHour - cfg.startHour) * 60;
        ...
        if (hpVal)
            hpVal.innerText = this.state.userHP.toString();
    }
    renderReflectionFact() {
        ...
        if (fact) {
            ...
            const hp = acts.reduce((s, t) => s + (t.is_recovery ? -t.cost.actual_hp : t.cost.actual_hp), 0);
            fact.innerHTML = `実行タスク： ${acts.length}<br>総HP消費量： ${hp}`;
            if (logs) {
                if (acts.length === 0)
                    logs.innerHTML = '<div class="hint">完了済みのタスクはありません</div>';
                else
                    logs.innerHTML = acts.map(t => `<div class="task-log-item" style="margin-bottom: 12px; border-left: 2px solid var(--accent-color); padding-left: 12px;"><div style="font-weight: 600; font-size: 0.9rem;">${t.title}</div><div style="font-size: 0.85rem; color: var(--text-secondary); margin-top: 4px;">${t.note || '(内省メモなし)'}</div></div>`).join('');
            }
        }
    }
    ...
    handleDeleteTask() { if (this.editingTaskId && confirm('削除しますか？')) {
    ...
    handleNodeAction(action) { if (this.selectedNode) {
        this.editingTaskId = null;
        this.creationMode = 'node';
        this.nodeEditMode = action;
        this.openCreationModal();
    } }
    openCreationModal(task) {
        ...
        if (this.creationMode === 'task') {
            ...
            if (subBtn)
                subBtn.innerText = task ? '更新' : '保存';
            document.getElementById('task-title').value = task ? task.title : '';
            document.getElementById('task-slot').value = task ? task.slot : this.currentTargetSlot;
            document.getElementById('task-importance').value = task ? task.importance : 'Mid';
            document.getElementById('task-goal-label').value = task ? (task.goal_label || '') : '';
            document.getElementById('task-deadline').value = task ? (task.deadline || '') : '';
            document.getElementById('task-estimated').value = task ? task.cost.estimated_time.toString() : '60';
            document.getElementById('task-mode').value = task ? (task.is_recovery ? 'recovery' : 'normal') : 'normal';
            document.getElementById('task-hp-est').value = task ? task.cost.hp_consumption.toString() : '10';
            document.getElementById('task-dod').value = task ? task.quality.dod : '';
        }
        else {
            if (delBtn)
            ...
            if (subBtn)
                subBtn.innerText = '保存';
            document.getElementById('node-text').value = this.nodeEditMode === 'edit' ? this.selectedNode.content : '';
        }
    }
    switchCreationTab(tab) {
    ...
    handleCreationSubmit() {
        if (this.creationMode === 'task') {
            ...
            const goal = document.getElementById('task-goal-label').value;
            const mode = document.getElementById('task-mode').value;
            const hp = parseInt(document.getElementById('task-hp-est').value);
            const est = parseInt(document.getElementById('task-estimated').value);
            const dod = document.getElementById('task-dod').value;
            if (this.editingTaskId) {
                ...
                if (t) {
                    ...
                    t.deadline = dead;
                    t.goal_label = goal;
                    t.is_recovery = mode === 'recovery';
                    t.cost.hp_consumption = hp;
                    t.cost.estimated_time = est;
                    t.quality.dod = dod;
                }
            }
            else {
    ...
    switchView(view) {
        ...
        if (view === 'mindmap')
            this.initMindMap('goal');
        if (view === 'reflection')
            this.initMindMap('analysis');
        this.hideNodeMenu();
    }
    initNavigation() {
    ...
    initMindMap(type) {
        ...
        this[key] = window.markmap.Markmap.create(sel, type === 'goal' ? { paddingX: 32 } : { paddingX: 20, autoFit: true, duration: 0 });
        this.updateMindMap(type);
    }
    updateMindMap(type) {
        ...
        if (type === 'goal')
            setTimeout(() => d3.selectAll('#markmap-svg-goal .markmap-node').on('click', (ev, d) => { ev.stopPropagation(); this.handleNodeClick(ev, d); }), 100);
    }
    handleNodeClick(ev, d) { this.selectedNode = d; const menu = document.getElementById('mm-node-actions'); if (menu) {
        menu.style.display = 'block';
        menu.style.left = `${ev.pageX + 10}px`;
        menu.style.top = `${ev.pageY - 20}px`;
    } }
    hideNodeMenu() { const menu = document.getElementById('mm-node-actions'); if (menu)
        menu.style.display = 'none'; this.selectedNode = null; }
    updateMarkdownState(md) { this.state.mindMapMarkdown = md; document.getElementById('mm-textarea').value = md; ASMOSStorage.saveState(this.state); this.updateMindMap('goal'); }
    ...
    closeModal(id) { document.getElementById(id).style.display = 'none'; if (id === 'creation-modal')
        this.hideNodeMenu(); }
    downloadMindMap() { const b = new Blob([this.state.mindMapMarkdown], { type: 'text/markdown' }); const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = `asmos_mindmap_${new Date().toISOString().split('T')[0]}.md`; a.click(); }
    handleMindMapUpload(e) { const f = e.target.files?.[0]; if (f) {
        const r = new FileReader();
        r.onload = (ev) => this.updateMarkdownState(ev.target?.result);
        r.readAsText(f);
    } }
    guiAddChild() {
        const newName = document.getElementById('node-text').value;
        if (!newName)
            return;
    ...
    guiDeleteNode() {
        ...
        for (let i = idx + 1; i < lines.length; i++)
            if (this.getLineLevel(lines[i]) > level)
                count++;
            else
                break;
    ...
    getLineLevel(line) { if (line.trim() === '')
        return 999; if (line.startsWith('#'))
        return line.match(/^#+/)?.[0].length || 0; return (line.match(/^ */)?.[0].length || 0) + 10; }
    updateTimeIndicator() {
        ...
        if (current < 5 || current > 22) {
            ind.style.display = 'none';
            return;
        }
        ind.style.display = 'block';
        let target = '';
        for (const id in TIME_SLOTS)
    ...
    openCompletionModal() { const modal = document.getElementById('completion-modal'); if (modal)
        modal.style.display = 'flex'; }
    handleModalSubmit() {
    ...
    downloadDailyReview() {
        ...
        a.download = `asmos_reflection_${d}.md`;
        a.click();
    }
}
