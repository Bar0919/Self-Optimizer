    ...
    static loadState() {
        ...
        if (data) {
            try {
                ...
                if (state.memoGood === undefined)
                    state.memoGood = '';
                if (state.memoImprove === undefined)
                    state.memoImprove = '';
                if (state.memoAnalysisMarkdown === undefined)
                    state.memoAnalysisMarkdown = '# 問題分析（ロジックツリー）\n\n## 今日の課題\n- なぜ起きた？\n  - 原因A\n  - 原因B';
                if (state.memoNextActions === undefined)
                    state.memoNextActions = '';
                if (state.memoInsight === undefined)
                    state.memoInsight = '';
                return state;
            }
            catch (e) {
...
class ASMOSApp {
    constructor() {
        ...
        this.currentTargetSlot = 'T1';
        this.currentTargetColumn = 'plan';
    ...
    bindEvents() {
        ...
        const handleImport = async (e) => {
            ...
            if (file) {
                try {
                    this.state = await ASMOSStorage.importState(file);
        ...
        const memoGood = document.getElementById('memo-good');
        const memoImprove = document.getElementById('memo-improve');
        const memoAnalysis = document.getElementById('memo-analysis-markdown');
        const memoNextActions = document.getElementById('memo-next-actions');
        const memoInsight = document.getElementById('memo-insight');
        memoGood?.addEventListener('input', (e) => { this.state.memoGood = e.target.value; ASMOSStorage.saveState(this.state); });
        memoImprove?.addEventListener('input', (e) => { this.state.memoImprove = e.target.value; ASMOSStorage.saveState(this.state); });
        memoAnalysis?.addEventListener('input', (e) => {
            this.state.memoAnalysisMarkdown = e.target.value;
            ASMOSStorage.saveState(this.state);
            this.updateMindMap('analysis');
        });
        memoNextActions?.addEventListener('input', (e) => { this.state.memoNextActions = e.target.value; ASMOSStorage.saveState(this.state); });
        memoInsight?.addEventListener('input', (e) => { this.state.memoInsight = e.target.value; ASMOSStorage.saveState(this.state); });
        this.setupRatingButtons('hp-rating');
        this.setupRatingButtons('quality-rating');
        const textarea = document.getElementById('mm-textarea');
        textarea?.addEventListener('input', (e) => {
            this.state.mindMapMarkdown = e.target.value;
            ASMOSStorage.saveState(this.state);
        ...
        document.addEventListener('click', (e) => {
            if (!e.target.closest('.markmap-node') && !e.target.closest('.node-actions-menu'))
                this.hideNodeMenu();
        });
    }
    initDragAndDrop() {
        ...
        document.addEventListener('dragenter', (e) => {
            const target = e.target.closest('.slot-column');
            if (target)
    ...
    handleTaskDrop(taskId, slotId, columnType) {
    ...
    handleAddTaskInline(slot, column) {
    ...
    handleAddTask() { this.creationMode = 'task'; this.openCreationModal(); }
    handleNodeAction(action) { if (!this.selectedNode)
        return; this.creationMode = 'node'; this.nodeEditMode = action; this.openCreationModal(); }
    openCreationModal() {
    ...
    initMindMap(type) {
        ...
        this[instanceKey] = window.markmap.Markmap.create(selector, { paddingX: 32 });
        this.updateMindMap(type);
    }
    updateMindMap(type) {
        ...
        if (type === 'goal') {
            setTimeout(() => {
                d3.selectAll('#markmap-svg-goal .markmap-node').on('click', (event, d) => { event.stopPropagation(); this.handleNodeClick(event, d); });
            }, 100);
        }
    }
    ...
    guiDeleteNode() {
        if (!confirm(`「${this.selectedNode.content}」を削除しますか？`))
            return;
        const lines = this.state.mindMapMarkdown.split('\n');
        const idx = lines.findIndex(l => l.includes(this.selectedNode.content));
        if (idx === -1)
    ...
    updateMarkdownState(newMd) {
        this.state.mindMapMarkdown = newMd;
        document.getElementById('mm-textarea').value = newMd;
        ASMOSStorage.saveState(this.state);
        this.updateMindMap('goal');
    }
    setupRatingButtons(groupId) {
    ...
    downloadMindMap() {
        ...
        a.download = `asmos_mindmap_${new Date().toISOString().split('T')[0]}.md`;
        a.click();
    }
    handleMindMapUpload(e) {
        const file = e.target.files?.[0];
        if (file) {
            const reader = new FileReader();
            reader.onload = (ev) => this.updateMarkdownState(ev.target?.result);
            reader.readAsText(file);
        }
    }
    downloadDailyReview() {
...
`;
        ...
        const a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = `asmos_daily_reflection_${date}.md`;
        a.click();
    }
}
